...
class DateFinder(object):
    ...
    def extract_date_strings(self, text, strict=False):
        ...
        for match in self.DATE_REGEX.finditer(text):
            ...
            if (len(strings_and_indexes) > 0):
                ...
                results.extend(strings_and_indexes)
                continue
            ...
            results.append((match_str, indices))
        return results

    def get_split_sentences_indexes(self, regex, string):
        indexes = []
        size = len(string)
        if not regex:
    ...
    def get_splitted_strings_with_indexes(self, regex, string, indices):
        indexes = [(indices[0] + k[0], indices[0] + k[1]) for k in self.get_split_sentences_indexes(regex, string)]
